...
//$data['data'] = pdo_fetchall("select s.openid from ims_sf_mini_sign s LEFT JOIN ims_sf_mini_user u ON s.openid=u.openid where s.addtime < $xtime AND s.uptime <> $uptime AND u.lasttime > $stime AND s.uniacid=:uniacid limit 100",array(':uniacid' =>$_W['uniacid']));
